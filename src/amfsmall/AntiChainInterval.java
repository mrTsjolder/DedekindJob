...
//		System.out.println((t++) + " : " + r);
	 ...
	 * @pre getTop().sp().max() < n
	 * Complexity (n n/2)
	 * @return getTop().join(getBottom().times{n}).standard(AntiChain.universe(n-1)).[x,y] where x and y are the relevant projections
	 */
	public AntiChainInterval fastStandard(Set<int[]>g, int n) {
		AntiChain chik;
		if (getBottom().isEmpty()) chik = getTop();
		else chik = getTop().join(getBottom().times(AntiChain.singletonFunction(n+1)));
		chik = chik.standard(g);
		AntiChain b = chik.meet(AntiChain.universeFunction(n));
		AntiChain a = new AntiChain();
