...
//		return count(a,b,c,rab,rac,rbc,pabacbc,abc); // this count is somewhat faster for n <= 4
	...
	/**
	 * count the number of ways to add sets from rxy to x and y while respectiong a.meet(b).meet(c).equals(abc)
	 * @param a
	 * @param b
	 * @param c
	 * @param rab sets for a of b
	 * @param rac
	 * @param rbc
	 * @param rabacbc sets that must occur twice
	 * @param abc
	 * @return
	 */
	@SuppressWarnings("unused")
	private static long count(AntiChain a, AntiChain b, AntiChain c,
			AntiChain rab, AntiChain rac, AntiChain rbc, AntiChain rabacbc,AntiChain abc) {
		if (!a.meet(b).meet(c).equals(abc)) return 0L; // impossible
		if (!rab.isEmpty()) {
			SmallBasicSet s = rab.first();
			AntiChain rabp = new AntiChain(rab);
			AntiChain ss = AntiChain.oneSetFunction(s);
			rabp.remove(s);
			return count(a.join(ss),b,c,rabp,rac,rbc,rabacbc,abc) + count(a,b.join(ss),c,rabp,rac,rbc,rabacbc,abc);
		} 
		if (!rac.isEmpty()) {
			SmallBasicSet s = rac.first();
			AntiChain racp = new AntiChain(rac);
			AntiChain ss = AntiChain.oneSetFunction(s);
			racp.remove(s);
			return count(a.join(ss),b,c,rab,racp,rbc,rabacbc,abc) + count(a,b,c.join(ss),rab,racp,rbc,rabacbc,abc);
		}
		if (!rbc.isEmpty()) {
			SmallBasicSet s = rbc.first();
			AntiChain rbcp = new AntiChain(rbc);
			AntiChain ss = AntiChain.oneSetFunction(s);
			rbcp.remove(s);
			return count(a,b.join(ss),c,rab,rac,rbcp,rabacbc,abc) + count(a,b,c.join(ss),rab,rac,rbcp,rabacbc,abc);
		}
		if (!rabacbc.isEmpty()) {
			SmallBasicSet s = rabacbc.first();
			AntiChain rabacbcp = new AntiChain(rabacbc);
			AntiChain ss = AntiChain.oneSetFunction(s);
			rabacbcp.remove(s);
			return count(a.join(ss),b.join(ss),c,rab,rac,rbc,rabacbcp,abc) 
					+ count(a.join(ss),b,c.join(ss),rab,rac,rbc,rabacbcp,abc) 
					+ count(a,b.join(ss),c.join(ss),rab,rac,rbc,rabacbcp,abc);
		}
		return 1L;
	}
